...
parameter  LOCAL_UDP_PORT_NUM = 16'h6102; //16'h5000;       
parameter  LOCAL_IP_ADDRESS   = 32'hc0a80003;//32'hc0a80002;//32'hc0a8f001;       
parameter  LOCAL_MAC_ADDRESS  = 48'h00_aa_bb_cc_dd_ee;//48'h0123456789ab;
parameter  DST_UDP_PORT_NUM   = 16'h5000;//16'h6102;       
parameter  DST_IP_ADDRESS     = 32'hc0a80002;//32'hc0a80003;//32'hc0a8f002;

wire [3:0]   led;
wire         app_rx_data_valid;//synthesis keep 
wire [7:0]   app_rx_data; //synthesis keep      
wire [15:0]  app_rx_data_length;//synthesis keep
wire [15:0]  app_rx_port_num;//synthesis keep
...
wire  [15:0] udp_data_length;//synthesis keep
...
//   for test
//----------------------------------------------------//
reg [7:0] app_rx_data_t;
reg        app_rx_data_valid_t;
reg [10:0]ctrl_cnt;
reg [23:0]ctrl_data;

//组合： SW2,SW1
//       00: 00FFFF 全关闭
//       01: FFFFFF 4个led 亮-绿，同时选择数码管对应87654321显示
//       10: F000FF 8个双色灯亮-黄
//       11: F0FF00 8个双色灯亮-红
//
always@(posedge udp_clk or posedge reset)
...
always@(*)
  begin
    case({sw2,sw1})
      2'b00: ctrl_data = 24'h00ffff;
      2'b01: ctrl_data = 24'hffffff;
      2'b10: ctrl_data = 24'hf000ff;
      2'b11: ctrl_data = 24'hf0ff00;
      default: ;
    endcase
  end
...
begin
    if(reset) 
    begin
      app_rx_data_valid_t <= 1'd0;
      app_rx_data_t <= 8'd0;
    end
    else if(ctrl_cnt==11'd1)
    begin
      app_rx_data_valid_t <= 1'd1;
      app_rx_data_t <= ctrl_data[23:16];
    end
    else if(ctrl_cnt==11'd2)
    begin
      app_rx_data_valid_t <= 1'd1;
      app_rx_data_t <= ctrl_data[15:8];
    end
    else if(ctrl_cnt==11'd3)
    begin
      app_rx_data_valid_t <= 1'd1;
      app_rx_data_t <= ctrl_data[7:0];
    end
    else
    begin
      app_rx_data_valid_t <= 1'd0;
...
udp_loopback#(
 ...
 (
    .app_rx_clk                 (udp_clk                ),
    .app_tx_clk                 (udp_clk                ),
    .reset                      (reset                  ),//(1'b1),//(reset                  ),
    
    `ifdef UDP_LOOP_BACK    
    .app_rx_data                (app_rx_data_t            ),//(app_rx_data            ),
    .app_rx_data_valid          (app_rx_data_valid_t      ),//(app_rx_data_valid      ),
    .app_rx_data_length         (16'h3     ),//(app_rx_data_length     ),
    `else   
...
(
    .DEVICE                     (DEVICE                 ),
    .LOCAL_UDP_PORT_NUM         (LOCAL_UDP_PORT_NUM     ),//(16'b0),//(LOCAL_UDP_PORT_NUM     ),
    .LOCAL_IP_ADDRESS           (LOCAL_IP_ADDRESS       ),//(32'b0),//(LOCAL_IP_ADDRESS       ),
    .LOCAL_MAC_ADDRESS          (LOCAL_MAC_ADDRESS      )//(48'b0) //(LOCAL_MAC_ADDRESS      )
)   
u3_udp_ip_protocol_stack    
(   
    .udp_rx_clk                 (udp_clk                ),
    .udp_tx_clk                 (udp_clk                ),//(1'b0),//(udp_clk                ),
    .reset                      (reset                  ), 
    .udp2app_tx_ready           (udp_tx_ready           ), //(),//(udp_tx_ready           ), 
    .udp2app_tx_ack             (app_tx_ack             ), //(),//(app_tx_ack             ), 
    .app_tx_request             (app_tx_data_request    ),//(1'b0),//(app_tx_data_request    ), 
    .app_tx_data_valid          (app_tx_data_valid      ),//(1'b0),//(app_tx_data_valid      ), 
    .app_tx_data                (app_tx_data            ),//(8'b0),//(app_tx_data            ), 
    .app_tx_data_length         (16'h3),//(udp_data_length        ),//(16'b0),//(udp_data_length        ), 
    .app_tx_dst_port            (16'd5000),//(16'd6102),//(DST_UDP_PORT_NUM       ), 
    .ip_tx_dst_address          (32'hc0a80002),//(32'hc0a80003),//(DST_IP_ADDRESS         ), 
    
    .input_local_udp_port_num      (16'd6102),//(16'd5000),//(input_local_udp_port_num      ),
    .input_local_udp_port_num_valid(1'b1),//(input_local_udp_port_num_valid),
    
    .input_local_ip_address     (32'hc0a80003),//(32'hc0a80002),//(32'hc0a8f001),//input_local_ip_address     ),
