 ...
 ** File	:	E:/lab202509/anlogic_fpga_design2025/al_ip/sd_fifo_sdr_data_2.v
 ** Date	:	2025 10 19
 ** TD version	:	5.6.119222
\************************************************************/
module sd_fifo_sdr_data_2 #(parameter DATA_WIDTH_W = 24, 
        parameter DATA_WIDTH_R = 24, 
        parameter ADDR_WIDTH_W = 12, 
        parameter ADDR_WIDTH_R = 12, 
        parameter AL_FULL_NUM = 4093, 
    ...
    output full_flag,  //fifo full flag
    output empty_flag,  //fifo empty flag
    output afull,  //fifo almost full flag
    ...
    output [(ADDR_WIDTH_R - 1):0] rdusedw //available data number for read      
        )  /* fehdl keep_hierarchy="true" */ ;
    //--------------------------------------------------------------------------------------------
    //-------------internal parameter and signals definition below---------------
    //--------------------------------------------------------------------------------------------
    //-------------parameter definition 
    localparam FULL_NUM = {ADDR_WIDTH_W{1'b1}} ; 
    //-------------signals definition
    reg asy_w_rst0 ; 
    reg asy_w_rst1 ; 
    reg asy_r_rst0 ; 
    ...
    assign full_flag = ((wr_addr_diff >= FULL_NUM) ? 1'b1 : 1'b0) ; 
    // generate fifo afull indicator
    assign afull = ((wr_addr_diff >= AL_FULL_NUM) ? 1'b1 : 1'b0) ; 
    ...
    fifo_cross_domain_addr_process_al_sd_fifo_sdr_data_2 #(.ADDR_WIDTH(ADDR_WIDTH_R)) rd_to_wr_cross_inst (.primary_asreset_i(rd_rst), 
    ...
    fifo_cross_domain_addr_process_al_sd_fifo_sdr_data_2 #(.ADDR_WIDTH(ADDR_WIDTH_W)) wr_to_rd_cross_inst (.primary_asreset_i(wr_rst), 
                .secondary_asreset_i(rd_rst), 
                .primary_clk_i(clkw), 
                .secondary_clk_i(clkr), 
                .primary_addr_i(wr_addr), 
                .secondary_addr_o(wr_to_rd_addr)) ; 
    ram_infer_sd_fifo_sdr_data_2 #(.DATAWIDTH_A(DATA_WIDTH_W),
...
`timescale 1ns/1ps
module fifo_cross_domain_addr_process_al_sd_fifo_sdr_data_2 #(parameter ADDR_WIDTH = 9) (
...
`timescale 1ns/1ps
module ram_infer_sd_fifo_sdr_data_2 (clka, 
        rsta, 
        cea, 
        ocea, 
        wea, 
